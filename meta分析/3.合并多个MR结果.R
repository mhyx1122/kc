...
# 输入CSV文件路径
meta_file <- "meta分析效应量示例数据.csv"

# 输入数据必须包含的列
required_cols <- c("Study", "beta", "se")

if (!file.exists(meta_file)) {
  stop("没有找到输入数据文件：", meta_file)
}
...
meta_data$beta <- suppressWarnings(as.numeric(meta_data$beta))
meta_data$se <- suppressWarnings(as.numeric(meta_data$se))

if (any(is.na(meta_data$Study)) || any(trimws(meta_data$Study) == "")) {
  stop("Study列存在缺失值或空值。")
}

if (any(is.na(meta_data$beta))) {
  stop("beta列存在非数值或缺失值。")
}

if (any(is.na(meta_data$se))) {
  stop("se列存在非数值或缺失值。")
}

if (any(meta_data$se <= 0)) {
  stop("se必须大于0。")
}
...
meta_data$lower <- meta_data$beta - 1.96 * meta_data$se
meta_data$upper <- meta_data$beta + 1.96 * meta_data$se
...
# 效应量名称，仅用于结果和图形标注
summary_measure <- "Beta"

# 是否计算固定效应模型和随机效应模型
common_model <- TRUE
random_model <- TRUE

# 随机效应置信区间方法，可设置为HK或classic
method_random_ci <- "HK"
...
main_result_table <- data.frame(
  ...
  Effect = c(
    if (common_model) m1$TE.common else NA_real_,
    if (random_model) m1$TE.random else NA_real_
  ),
  Lower95CI = c(
    if (common_model) m1$lower.common else NA_real_,
    if (random_model) m1$lower.random else NA_real_
  ),
  Upper95CI = c(
    if (common_model) m1$upper.common else NA_real_,
    if (random_model) m1$upper.random else NA_real_
  ),
  Pvalue = c(
...
write.csv(
  main_result_table,
  file.path(out_dir, "IVW_main_result_table.csv"),
  row.names = FALSE
)

write.csv(
  meta_result,
  file.path(out_dir, "IVW_meta_summary_beta.csv"),
  row.names = FALSE
)

write.csv(
  meta_data,
  file.path(out_dir, "IVW_uploaded_data_checked.csv"),
  row.names = FALSE
)
...
heterogeneity_result_text <- if (low_heterogeneity) {
  ...
  "研究间存在一定异质性，解释结果时需结合研究设计、样本来源、工具变量选择等差异谨慎判断。"
}
...
interpretation_text <- paste0(
  ...
  "6. 若逐一剔除任一研究后总体方向基本一致，说明研究间Meta结果稳健性较好；",
...
if (inherits(tf, "error")) {
  ...
  trimfill_effect <- if (random_model) {
    tf$TE.random
  } else {
    tf$TE.common
  }
  ...
  trimfill_text <- paste0(
    ...
    "3. Trim-and-fill校正后", model_used,
    "合并值约为：", round(trimfill_effect, 4), "\n",
...
# X轴标题
forest_xlab <- "Inverse-variance pooled Beta"
...
# X轴标题
metainf_xlab <- "Beta after omitting one study"
...
meta::forest(
  ...
  xlab = metainf_xlab,
  col = metainf_col,
  col.bg = metainf_col_bg,
  col.border = metainf_col_border
)
...
funnel_pch <- 19
funnel_xlab <- "Inverse-variance pooled Beta"
...
parameter_text <- paste0(
  "运行流程说明：\n",
  "1. 使用普通R脚本读取各研究独立完成后的效应估计数据。\n",
  "2. 使用meta包的metagen()对各研究提供的beta与标准误se进行研究间Meta分析。\n",
  ...
  "写作提示词：\n",
  "1. 基于各研究独立完成的效应估计值beta及其标准误，采用Meta分析方法进行研究间合并。\n",
  "2. 结合Q检验、I²和Tau²评估研究间异质性，并据此解释优先参考的模型结果。\n",
  "3. 结合森林图展示各研究效应方向及总体合并效应。\n",
  "4. 通过逐一剔除敏感性分析评估研究间Meta结果的稳健性。\n",
  "5. 结合漏斗图、Egger检验、Begg检验及Trim-and-fill分析，对发表偏倚进行辅助判断。\n",
  "6. beta大于0通常提示暴露增加与结局升高相关，beta小于0通常提示暴露增加与结局降低相关。"
)

writeLines(
  parameter_text,
  file.path(out_dir, "IVW_meta_parameters.txt")
)

message("IVW法Meta分析完成。")
message(
  "结果保存位置：",
