...
# 8. 创建结果目录
if (!dir.exists("1.差异分析/TPM")) {
  dir.create("1.差异分析/TPM", recursive = TRUE)
}
...
# 10. log2转化箱线图
pdf("1.差异分析/TPM/1.log2转化前箱线图.pdf", width = 9, height = 6)
par(mar = c(7, 7, 2, 2))
...
if (LogC) {
  expMatrix <- log2(ex + 1)
  pdf("1.差异分析/TPM/2.log2转化后箱线图.pdf", width = 9, height = 6)
  par(mar = c(7, 7, 2, 2))
...
write.csv(limma_DEG, file = "1.差异分析/TPM/limma差异分析结果.csv", row.names = TRUE)

limma_log2FC_TPM <- data.frame(gene_symbol = rownames(limma_DEG), change = limma_DEG$change, logFC = limma_DEG$logFC)
write.csv(limma_log2FC_TPM, file = "1.差异分析/TPM/limma_log2FC.csv", row.names = FALSE)

limma_log2FC_fpkm_filtered <- limma_log2FC_TPM %>%
  filter(change != "NOT")
write.csv(limma_log2FC_fpkm_filtered, file = "1.差异分析/TPM/limma_上下调基因.csv", row.names = FALSE)
...
write.csv(RankTest, file = "1.差异分析/TPM/Wilcoxon秩和检验结果.csv")
...
RankTest_log2FC_TPM <- data.frame(gene_symbol = rownames(RankTest), change = RankTest$change, logFC = RankTest$logFC)
write.csv(RankTest_log2FC_TPM, file = "1.差异分析/TPM/WilcoxTest_log2FC.csv", row.names = FALSE)

limma_log2FC_fpkm_filtered <- RankTest_log2FC_TPM %>%
  filter(change != "NOT")
write.csv(limma_log2FC_fpkm_filtered, file = "1.差异分析/TPM/Wilco_Test_上下调基因.csv", row.names = FALSE)
...
ggsave("1.差异分析/TPM/3.limma单独热图.pdf", plot = p1, width = pdf_width, height = pdf_height, units = "cm")

v1 <- draw_volcano(limma_DEG, pkg = 4, logFC_cutoff = logFC_cutoff, color = volcano_color, adjust = adjust)
ggsave("1.差异分析/TPM/4.limma单独火山图.pdf", plot = v1, width = pdf_height, height = pdf_height, units = "cm")

pdf("1.差异分析/TPM/5.组合作图.pdf", width = 10, height = 5)
...
ggsave("1.差异分析/TPM/6.Wilcox单独热图.pdf", plot = p6, width = pdf_height, height = pdf_height, units = "cm")
...
ggsave("1.差异分析/TPM/7.Wilcox单独火山图.pdf", plot = v1, width = pdf_height, height = pdf_height, units = "cm")

pdf("1.差异分析/TPM/8.Wilcox组合作图.pdf", width = 10, height = 5)
par(mar = c(bottom, left, top, right))
plot(v1 + h1)
dev.off()