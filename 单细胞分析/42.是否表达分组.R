...
plot_width <- 8
plot_height <- 6
output_dir <- "特定细胞群的单基因分组"

if (!dir.exists(output_dir)) dir.create(output_dir, recursive = TRUE)
if (!(gene_name %in% rownames(seurat))) stop(paste0("目标基因 ", gene_name, " 不存在于当前默认 assay 中。"))
if (!(cell_name %in% unique(as.character(seurat$cellType)))) stop(paste0("cellType 列中不存在指定细胞类型：", cell_name))


# 4. 提取指定细胞群的细胞条形码

target_cells <- rownames(seurat@meta.data)[as.character(seurat$cellType) == cell_name]
...
gene_expr <- FetchData(seurat, vars = gene_name, assay = "RNA", layer = "data")[target_cells, , drop = FALSE]

gene_expr$gene_group <- ifelse(gene_expr[, 1] > 0, high_group_name, low_group_name)

print(table(gene_expr$gene_group))


# 6. 更新指定细胞群的 cellType

seurat@meta.data$cellType <- as.character(seurat@meta.data$cellType)
seurat@meta.data$cellType[match(rownames(gene_expr), rownames(seurat@meta.data))] <- gene_expr$gene_group
...
pdf(file.path(output_dir, "单基因是否表达该基因(umap).pdf"), width = plot_width, height = plot_height)
...
pdf(file.path(output_dir, "单基因是否表达该基因(tsne).pdf"), width = plot_width, height = plot_height)
